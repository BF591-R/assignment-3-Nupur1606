...
if (nrow(long_format_data) > 0) {
  ggplot(long_format_data, aes(x = gene_set, y = value, fill = gene_set)) +
    ...
    scale_fill_manual(values = c("good" = "#2A9D8F", "bad" = "#E76F51")) +
    theme_minimal(base_size = 12) +
    labs(
      title = "Expression levels by gene set",
      x = "Gene set",
      y = "Expression value"
    ) +
  ...
  ggplot() +
    theme_void() +
    labs(title = "Boxplot skipped: no mapped genes available (Ensembl blocked here)")
}
